...
echo ""
echo "MEMORY WARNING (measured on the reference agent sandbox): the harness"
echo "stores one Duration (16 bytes) per timed iteration PER THREAD"
echo "(FullPipeline) and prebuilds ~88 bytes/iteration per thread"
echo "(VerdictOnly). With ~0.4-0.5 us/op this is roughly 3-8 GB per thread"
echo "in FullPipeline and ~17-43 GB per thread in VerdictOnly: check free"
echo "RAM and available swap before launching, or expect the OOM killer."
echo ""
echo "Running sweep — this will take multiple hours. Output: ${CSV_PATH}"

cargo run --release --bin sweep_concurrent > "${CSV_PATH}"
