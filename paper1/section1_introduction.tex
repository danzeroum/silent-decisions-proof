...
Linear Logic~\cite{girard1987linear,wadler1991linear} treats
propositions as \emph{resources} used exactly once, neither duplicated
nor discarded. We encode the accountability invariant directly in the
type system as $V \multimap (E \otimes C)$: a Verdict~($V$) can only be
produced by linearly consuming an \texttt{EvidenceToken}~($E$) and a
\texttt{ComplianceToken}~($C$) together (\S\ref{sec:type_system}
develops the semantics). Concretely, \texttt{EvidenceToken} is
\texttt{\#[must\_use]} with no \texttt{Clone}/\texttt{Copy};
\texttt{Verdict::new()} consumes both tokens by value; and
\texttt{Blake3Hash} has no public constructor, so it cannot be forged
outside \texttt{EvidenceToken::consume()}. A silent decision is
therefore not an illegal act that slips past a guard but a
\emph{type error}: the compiler rejects it before any evidence is
dropped.
...
\begin{enumerate}
  ...
  \item \textbf{The Constitutional Enclosure Theorem (C3).}
    We state and prove that in any Safe Rust system satisfying the BTV
    type invariant, the set of materialized silent decisions is empty
    within the crate's type perimeter (\S\ref{sec:encapsulation} states
    the scope precisely).
    ...
    verifies all seventeen clauses in under four seconds.\
\end{enumerate}
