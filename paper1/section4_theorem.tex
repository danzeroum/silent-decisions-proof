...
The theorem holds under the \emph{Safe Rust} memory model; the
\texttt{unsafe} keyword can bypass borrow-checker and visibility
rules, so \texttt{unsafe} code is explicitly out of scope. This is not
a practical limitation: \texttt{cargo geiger}~\cite{cargogeiger}
reports every \texttt{unsafe} block transitively reachable from the
dependency graph, and a zero-\texttt{unsafe} audit guarantees the
theorem applies to the entire compiled binary. Macro-generated code is
in scope: procedural macros run before the type checker and cannot
circumvent visibility rules without themselves using \texttt{unsafe}.
...
unavailable context), the type system provides exactly one
well-typed alternative: constructing an \texttt{EscalatedVerdict}
via a linearly-consumed \texttt{OperatorToken}.
Silent fallback to an unevidenced automated verdict is ill-typed.
Formally, $V_{\mathrm{esc}} \multimap (O \otimes \mathbf{1})$.
...
human operator, whose identity and authorisation are
cryptographically sealed via HMAC.
\end{corollary}
...
(x86-64; Section~\ref{sec:results}): the escalation path is $24$ bytes
\emph{smaller}, since \texttt{EscalatedVerdict} carries an
\texttt{OperatorToken}'s identity and signature directly rather than a
full \texttt{ComplianceToken} (which itself carries an authority
signature, \S\ref{sec:results}). The relative size is a byproduct of
which fields each type embeds, not a claim either construction is
cheaper in the sense that matters for the proof.
...
verify by exhaustive case analysis (mirroring Cases~A, B, and~C
