...
\end{enumerate}
Together, these restrictions close the encapsulation perimeter.
\end{axiom}
...
crate's encapsulation assumptions (Figure~\ref{fig:enclosure_perimeter}).
...
\caption{The Constitutional Enclosure perimeter: attack vectors A--C are
  rejected by the compiler (\texttt{E0451}, \texttt{E0624},
  \texttt{unused\_must\_use}); the only well-typed path to a
  \texttt{Verdict} is \texttt{Verdict::new}.}
...
\begin{proof}
The only well-typed continuation that produces a \texttt{Verdict}
requires unwrapping the result and handling the error branch
explicitly; that branch must either propagate the error or halt.  By
...
exercise the corresponding encapsulation and ownership cases of
Theorem~\ref{thm:enclosure}; no \texttt{EscalatedVerdict} can be
forged, duplicated, or silently dropped.
\end{proof}
...
compiler (Figure~\ref{fig:ffi_boundary}).
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig5_ffi_boundary.pdf}
\caption{The FFI trust boundary in a polyglot deployment. The Rust
  gateway is the only path to a sealed \texttt{Verdict} handle; routing
  every downstream effect through that handle is an infrastructure
  obligation outside the Rust type theorem.}
\label{fig:ffi_boundary}
\end{figure}
