...
This distinction between the crate's linear discipline and Rust's own
ownership model is precise enough to need its own name; we return to
it as ``affine versus linear'' below (Section~\ref{sec:affine-linear}).
...
This construction property is stronger than a call-site runtime assertion.
Such an assertion executes only after the call site is reached and can be
omitted from an untested path.  The ownership constraint is enforced by the type
...
C & Silent token drop & \texttt{\#[must\_use]} & \texttt{unused\_must\_use} \\
\hline
\end{tabular}
\end{table}
...
they reflect a deliberate separation of concerns.  The public API
enforces a \emph{structural} property---that a BTV \texttt{Verdict} cannot
be constructed without the required resources.  \emph{Substantive} accountability---that
