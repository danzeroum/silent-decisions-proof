...
\paragraph{Affine versus linear (Section~\ref{sec:affine-linear}).}
\label{sec:affine-linear}
Rust's ownership is \emph{affine}: a value may be used zero or one
times, not exactly once
\cite{walker2005substructural, ahmed2009state}.
BTV therefore recovers full linearity \emph{at the API surface}: every
token is \texttt{\#[must\_use]}, the crate compiles with
\texttt{\#![deny(unused\_must\_use)]}, and the sole destructors are
\texttt{pub(crate)}---so silently discarding a token is a compile
error that cannot be discharged outside the defining crate.  Three
escape hatches remain, documented rather than denied:
\texttt{mem::forget} stays callable on tokens the caller owns
(leaking, not duplicating, the obligation); a panic between creation
and consumption drops the token during unwinding; process abort loses
it with the process.  None produces a materialized unevidenced
\texttt{Verdict}; they bound the guarantee's \emph{liveness}, not its
...
B & External \texttt{consume()} call &
  \texttt{pub(crate)} visibility & \texttt{E0624} \\
...
\begin{itemize}
  ...
  \item \textbf{Out of scope:} Decisions that bypass the
    ...
    \texttt{Verdict}.  Note that \texttt{let \_ =
    EvidenceToken::new(\ldots)} silences the \texttt{\#[must\_use]}
    lint; however, this produces no \texttt{Verdict} and therefore
    does not constitute a silent decision under
    Definition~\ref{def:silent}.  Enforcing that all decision points
    use the BTV pipeline is an architectural obligation, not a
    type-level one.
\end{itemize}
