...
% ── Attack vector C (from the north-east): token drop ────────────────────
\node[att, above right=14mm and 2mm of perim.north east, anchor=south] (a3)
  {Case C: token\\ silently dropped};
\draw[blk] (a3.south) -- ($(perim.north east)+(-6mm,0)$)
  node[midway, right, note] {\texttt{unused\_must\_use}};
\node[draw=black!60, cross out, semithick, inner sep=1.6pt]
  at ($(perim.north east)+(-6mm,0)$) {};
