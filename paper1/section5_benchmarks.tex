...
The implementation was compiled with \texttt{rustc}~1.98.1
(pinned as the single toolchain source in \texttt{rust-toolchain.toml};
...
\begin{center}
\texttt{std::mem::size\_of::\textless{}Verdict\textgreater{}() = 184 bytes} (x86-64, rustc~1.98.1, Linux)
\end{center}
This fixed footprint comprises the 32-byte \texttt{Blake3Hash}, the
\texttt{ComplianceToken} fields (two heap-allocated strings, a
\texttt{u32}, and the 32-byte authority signature introduced when the
compliance token became cryptographically signed), the \texttt{Decision}
discriminant, the explanation \texttt{String}, and the 32-byte HMAC
integrity seal.  The serialised
