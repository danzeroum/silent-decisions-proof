...
\begin{enumerate}
  ...
  \item Direct invocation of \texttt{EvidenceToken::consume()}---the
    method is \texttt{pub(crate)} (compiler error \texttt{E0603}).
  \item Construction of \texttt{Blake3Hash} via its tuple-struct
    field---the field is private (compiler error \texttt{E0603}).
\end{enumerate}
...
\texttt{error[E0603]: function `consume` is private}.
This is verified by the compile-fail test
\texttt{tests/ui/blake3hash\_public\_constructor.rs}.
...
hard error.  The developer cannot proceed without either consuming
the token (producing a materialized verdict) or explicitly
acknowledging the drop (which requires modifying the crate's internal
visibility rules---itself an auditable action outside normal
...
Theorem~\ref{thm:enclosure}, a runtime failure to produce evidence
(e.g., a database timeout, a missing context field) propagates
as a type error at compile time rather than as a silent omission
at runtime.  The system \emph{fails securely}: it cannot issue a
decision that lacks an evidence chain.
\end{corollary}
